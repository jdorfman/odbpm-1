...
function run_tests {
  ...
  assert_file "/tmp/.odbpm.*/odbpm-test-pkg/test.sh" "_fetch_mktmp: extract repo"

  # cleanup
  _fetch_cleanup
  refute_file "/tmp/.odbpm.*/master.zip" "_fetch_cleanup: fetched repo"
  refute_file "/tmp/.odbpm.*/odbpm-test-pkg/test.sh" "_fetch_cleanup: extracted repo"

  # with version
  config[repo]="jmervine/odbpm-test#0.0.1"
  _fetch_repo
  assert_file "/tmp/.odbpm.*/0.0.1.zip" "_fetch_repo: fetch repo with version"
  assert_file "/tmp/.odbpm.*/odbpm-test-pkg/test.sh" "_fetch_repo: extract repo with version"
  assert_grep "cat /tmp/.odbpm.*/odbpm-test-pkg/test.sh" "branch" "_fetch_repo: fetch correct version"
